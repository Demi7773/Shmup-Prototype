...
public class SpawnerLevel2 : SimpleEnemySpawner
{
    ...
    [Space(10)]
    [Header("Wave4")]
    [SerializeField] private Transform _parentForWave4;
    [SerializeField] private int _zoomerSpawnsForWave4 = 20;
    [SerializeField] private int _boomerSpawnsForWave4 = 15;
    [SerializeField] private int _shooterSpawnsForWave4 = 10;
    private int _indexWave4 = 0;
    [SerializeField] private List<EnemyBehavior> _wave4ToShuffle = new List<EnemyBehavior>();
    [SerializeField] private List<EnemyBehavior> _wave4Shuffled = new List<EnemyBehavior>();
    [Space(10)]
    [Header("Wave5")]
    [SerializeField] private Transform _parentForWave5;
    [SerializeField] private int _zoomerSpawnsForWave5 = 25;
    [SerializeField] private int _boomerSpawnsForWave5 = 20;
    [SerializeField] private int _shooterSpawnsForWave5 = 15;
    private int _indexWave5 = 0;
    [SerializeField] private List<EnemyBehavior> _wave5ToShuffle = new List<EnemyBehavior>();
    [SerializeField] private List<EnemyBehavior> _wave5Shuffled = new List<EnemyBehavior>();
    ...
    // FireMe sequence, currently tightly coupled with base abstract class
    ...
    private void InitWave4(Transform player)
    {
        for (int i = 0; i < _zoomerSpawnsForWave4; i++)
        {
            var enemy = Instantiate(_zoomerPrefab, _parentForWave4);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);

            _wave4ToShuffle.Add(enemy);
        }
        for (int i = 0; i < _boomerSpawnsForWave4; i++)
        {
            var enemy = Instantiate(_boomerPrefab, _parentForWave4);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);

            _wave4ToShuffle.Add(enemy);
        }
        for (int i = 0; i < _shooterSpawnsForWave4; i++)
        {
            var enemy = Instantiate(_shooterPrefab, _parentForWave4);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);
        ...
        ShuffleList(_wave4ToShuffle, _wave4Shuffled);
    }
    private void InitWave5(Transform player)
    {
        for (int i = 0; i < _zoomerSpawnsForWave5; i++)
        {
            var enemy = Instantiate(_zoomerPrefab, _parentForWave5);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);

            _wave5ToShuffle.Add(enemy);
        }
        for (int i = 0; i < _boomerSpawnsForWave5; i++)
        {
            var enemy = Instantiate(_boomerPrefab, _parentForWave5);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);

            _wave5ToShuffle.Add(enemy);
        }
        for (int i = 0; i < _shooterSpawnsForWave5; i++)
        {
            var enemy = Instantiate(_shooterPrefab, _parentForWave5);
            int lvl = GetEnemyLevelRoll();
            enemy.Init(lvl, player);
        ...
        ShuffleList(_wave5ToShuffle, _wave5Shuffled);
    }
    ...
    private void Wave4SpawnEnemy()
    {
        var enemy = _wave4Shuffled[_indexWave4];
        Vector3 spawnPos = GetSpawnPos();
        enemy.SpawnMeAt(spawnPos);
        _livingEnemies.Add(enemy);

        _indexWave4++;
        if (_indexWave4 > _wave4Shuffled.Count - 1)
        {
            _enemyManager.StopSpawning();
            Debug.Log("Reached end of Wave4 spawns at: " + _indexWave4);
        }
    }
    private void Wave5SpawnEnemy()
    {
        var enemy = _wave5Shuffled[_indexWave5];
        Vector3 spawnPos = GetSpawnPos();
        enemy.SpawnMeAt(spawnPos);
        _livingEnemies.Add(enemy);

        _indexWave5++;
        if (_indexWave5 > _wave5Shuffled.Count - 1)
        {
            _enemyManager.StopSpawning();
            Debug.Log("Reached end of Wave5 spawns at: " + _indexWave5);
        }
    }
